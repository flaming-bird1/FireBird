                ...
                <div class="search-bar">
                  <div class="search-input-wrapper">
                    ...
                    <input
                        ...
                        placeholder="搜索日期（如 04-01）或周总结内容..."
                        class="search-input"
                    />
 ...
 * 支持多种日期格式输入：
 *
 * 04-01 或 4/1 → 匹配周开始或结束日期
 *
 * 03/31-04/06 → 匹配完整的周范围
 *
 * 3月31日 → 中文日期格式
 *
 * 0331 → 紧凑格式
...
const filteredWeeklySummaries = computed(() => {
  ...
  return weeklySummaries.value.filter(week => {
    // 搜索周范围日期（支持多种格式）
    // 格式如：03/31-04/06、3/31-4/6、0331-0406、3月31日 等
    const weekStartDate = new Date(week.weekStart)
    const weekEndDate = new Date(week.weekEnd)

    // 生成多种日期格式用于搜索
    const startStr1 = formatShortDate(week.weekStart).toLowerCase() // MM/DD
    const startStr2 = formatShortDateNoLeadingZero(week.weekStart).toLowerCase() // M/D
    const startStr3 = formatFullDate(week.weekStart).toLowerCase() // M月D日
    const startStr4 = week.weekStart.slice(5).replace('-', '') // MMDD

    const endStr1 = formatShortDate(week.weekEnd).toLowerCase()
    const endStr2 = formatShortDateNoLeadingZero(week.weekEnd).toLowerCase()
    const endStr3 = formatFullDate(week.weekEnd).toLowerCase()
    const endStr4 = week.weekEnd.slice(5).replace('-', '')

    // 组合成周范围字符串，如 "03/31-04/06"
    const rangeStr1 = `${startStr1}-${endStr1}`
    const rangeStr2 = `${startStr2}-${endStr2}`
    const rangeStr3 = `${startStr3}-${endStr3}`
    const rangeStr4 = `${startStr4}-${endStr4}`

    // 检查是否匹配日期
    const dateMatch =
        rangeStr1.includes(keyword) ||
        rangeStr2.includes(keyword) ||
        rangeStr3.includes(keyword) ||
        rangeStr4.includes(keyword) ||
        startStr1.includes(keyword) ||
        startStr2.includes(keyword) ||
        startStr3.includes(keyword) ||
        startStr4.includes(keyword) ||
        endStr1.includes(keyword) ||
        endStr2.includes(keyword) ||
        endStr3.includes(keyword) ||
        endStr4.includes(keyword)
...
// 不带前导零的短日期格式，如 "3/31"
const formatShortDateNoLeadingZero = (date: string) => {
  const d = new Date(date)
  return `${d.getMonth() + 1}/${d.getDate()}`
}
// 将文本按换行符分割成段落，并过滤掉空行
