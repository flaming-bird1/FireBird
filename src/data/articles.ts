...
export const articles: Article[] = [
    ...
    },
    {
        ...
        title: '操作系统-第三章 内存管理',
        date: '2026-08-04',
        description: '本文系统梳理了操作系统内存管理的核心知识：从逻辑地址与物理地址的映射原理（基址寄存器+MMU）、程序的链接与装入三种方式，到内存保护、共享、分配与回收的完整闭环。通过乐高积木、小区分房等生活化类比，生动讲解内存管理从单一连续区、固定分区、动态分区到分页、分段、虚拟内存的演进路线，并附大量对比表格，是一份适合操作系统复习备考的图文笔记。',
        tags: ['操作系统', '内存管理'],
        // 未配置封面，自动使用默认封面图
        fileName: '操作系统-第三章 内存管理', // 对应的Markdown文件名
        fileType: 'md'  // 明确指定文件类型
    },
